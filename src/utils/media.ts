...
export async function getImages(options: Omit<UnresolvedImageTransform, 'src'>): Promise<ImageData[]> {
  return Promise.all(getImagesMetadata().map(async ([key, src]) => [key, await getImage({ ...options, src })]))
}

export function getVideos(): VideoData[] {
  const videoModules = import.meta.glob('/src/assets/media/*.{mp4,webm}', { eager: true }) as Record<string, { default: string }>
